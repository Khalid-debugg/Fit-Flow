...
export default function CreateMember({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else toast.error(t('members.errors.createFailed'))
    }
  }
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogTrigger asChild>
        <Button className="gap-2" variant="secondary">
          <Plus className="h-4 w-4" /> {t('members.addNew')}
        </Button>
      </DialogTrigger>
      <DialogContent className="bg-gray-900 border-gray-700 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('members.addNew')}</DialogTitle>
        </DialogHeader>
        <MemberForm
          formData={formData}
          ...
          submitLabel={t('members.form.submit')}
        />
      </DialogContent>
    </Dialog>
  )
